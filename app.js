...
app.post('/weather', function(req, res, next) {
    ...
    const json_obj = getJSON(`http://api.openweathermap.org/data/2.5/weather?q=${getCity[1]}&APPID=fc2a5047efd117936135c68fe985dcf6&units=metric`,
    function(err, data) {
      if (err != null) {
        console.log('Something went wrong')
      } else {
        const botPayLoad = {
            text: `Hello ${userName}, the weather for ${getCity[1]} is: \n TEMP - ${json_obj.main.temp} \n WEATHER - ${json_obj.weather.description}`
        };
      }
    });
